/* ***********************************************
Author :111qqz
Created Time :Thu 03 Nov 2016 08:41:40 AM CST
File Name :code/acdream/1075.cpp
************************************************ */
...
LL find_loop(LL n)
{
    ...
    {
        LL rec = 1;
        if (pri[i]<=a*a+4*b) rec = get_loop(pri[i]);
        else
        {
            if (legendre(a*a+4*b,pri[i])==1)
                get_fac(pri[i]-1);
            else get_fac((pri[i]+1)*2);
            sort(fac,fac+cnt2);
            for ( int j = 0 ; j < cnt2 ; j++) //挨个验证因子
            {
                ...
                if (tmp.mat[0][0]==c&&tmp.mat[1][0]==d)
                {
                    rec = fac[j];
                    break;
                }
